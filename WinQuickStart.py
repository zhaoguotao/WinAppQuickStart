...
os.popen(cmd)无法在纯GUI程序中使用，就是说只能用pyinstaller -F -c 生成exe，如果用-w，则os.popen(cmd)命令无法执行。而os.system(cmd)可以使用-w编译成exe。
'''
import os
import time
import platform
...
def Callback_controlfolders():
    cmd = "control folders"
#     os.startfile(cmd)
    os.system(cmd)
    
def Callback_NetworkAndSharingCenter():
    cmd = "control.exe /name Microsoft.NetworkAndSharingCenter"
    os.system(cmd)
    
def Callback_snippingtool():
...
btn_names_6 = ["声音","程序和功能","文件夹选项"]
btn_names_7 = ["截图工具","cmd","网络和共享中心"]
...
btn_func_6  = [Callback_mmsys,Callback_appwiz,Callback_controlfolders]
btn_func_7  = [Callback_snippingtool,Callback_cmd,Callback_NetworkAndSharingCenter]
...
if __name__ == '__main__':
    ...
    BTN1.grid(row=m+1,column=0, ipady=2)
    ttk.Button(top, text ="本程序路径", command = Callback_OpenAppFolder,width=13).grid(row=m+1,column=1, ipady=2)
    ttk.Button(top, text ="显示标题", command = Callback_overrideredirect,width=13).grid(row=m+1,column=2, ipady=2)
